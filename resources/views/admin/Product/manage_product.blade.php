                    ...
                    <div class="card-body">
                        <div class="border p-4 rounded">
                            ...
                            <div class="row mb-3">
                                ...
                                <div class="col-sm-9">
                                    <input type="file" class="form-control" name="image" id="inputEnterYourName" placeholder="Enter Product Image">
                                    @if($data->id && $data->image)
                                        <div class="mt-2">
                                            <img src="{{ asset($data->image) }}" alt="Current Image" style="width: 150px; height: 150px; object-fit: cover;" class="img-thumbnail">
                                ...
                                <div class="col-sm-9">
                                    <select name="tax_id" class="form-control" id="">
                                        ...
                                        @foreach ($tax as $taxList)
                                           <option value="{{ $taxList->id }}" {{ ($data->tax_id == $taxList->id) ? 'selected' : '' }}>{{ $taxList->text }}%</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                                ...
                                <div class="" id="addAttributeContent">
                                    @if($product_attr->count() > 0)
                                        @foreach($product_attr as $index => $attr)
                                            <div class="attribute-item mt-4" data-attr-index="{{ $index }}">
                                                <div class="row">
                                                            ...
                                                            <div class="col-sm-3 mb-3">
                                                                <select name="color[]" class="form-control">
                                                                    @foreach ($color as $colorList)
                                                                       <option value="{{ $colorList->id }}" {{ ($attr->color_id == $colorList->id) ? 'selected' : '' }} style="background-color:{{ $colorList->value }};color: #fff;" class="box_color">{{ $colorList->color }}</option>
                                                                    @endforeach
                                                                </select>
                                                            </div>
                                                            <div class="col-sm-3 mb-3">
                                                                <select name="size[]" class="form-control">
                                                                    @foreach ($size as $sizeList)
                                                                       <option value="{{ $sizeList->id }}" {{ ($attr->size_id == $sizeList->id) ? 'selected' : '' }} class="box_color">{{ $sizeList->size }}</option>
                                                                    @endforeach
                                                                </select>
                                                            </div>
                                                            <div class="col-sm-3 mb-3">
                                                                <input type="text" name="sku[]" class="form-control" placeholder="Enter SKU" value="{{ $attr->sku }}" />
                                                            </div>
                                                            <div class="col-sm-3 mb-3">
                                                                <input type="text" name="mrp[]" class="form-control" placeholder="Enter MRP" value="{{ $attr->mrp }}" />
                                                            </div>
                                                            <div class="col-sm-3 mb-3">
                                                                <input type="text" name="price[]" class="form-control" placeholder="Enter Price" value="{{ $attr->price }}" />
                                                            </div>
                                                            <div class="col-sm-3 mb-3">
                                                                <input type="text" name="length[]" class="form-control" placeholder="Enter Length" value="{{ $attr->length }}" />
                                                            </div>
                                                            <div class="col-sm-3 mb-3">
                                                                <input type="text" name="breadth[]" class="form-control" placeholder="Enter Breadth" value="{{ $attr->breadth }}" />
                                                            </div>
                                                            <div class="col-sm-3 mb-3">
                                                                <input type="text" name="height[]" class="form-control" placeholder="Enter Height" value="{{ $attr->height }}" />
                                                            </div>
                                                            <div class="col-sm-3 mb-3">
                                                                <input type="text" name="weight[]" class="form-control" placeholder="Enter Weight" value="{{ $attr->weight }}" />
                                                            </div>
                                                            <div class="row mb-3">
                                                                ...
                                                                <div class="col-sm-9">
                                                                    ...
                                                                    @if(isset($product_attr_images[$attr->id]) && $product_attr_images[$attr->id]->count() > 0)
                                                                        <div class="mb-3">
                                                                            <p class="text-muted small">Existing Images:</p>
                                                                            <div class="d-flex flex-wrap gap-2 existing-images-container" data-attr-id="{{ $attr->id }}">
                                                                                @foreach($product_attr_images[$attr->id] as $img)
                                                                                    <div class="position-relative existing-image-item" style="width: 100px;" data-image-id="{{ $img->id }}">
                                                                                        <img src="{{ asset($img->image) }}" alt="Existing Image" style="width: 100px; height: 100px; object-fit: cover;" class="img-thumbnail">
                                                                                        <button type="button" class="btn btn-danger btn-sm position-absolute top-0 end-0 remove-existing-image" style="width: 24px; height: 24px; padding: 2px; border-radius: 50%; display: flex; align-items: center; justify-content: center;">
                                                                                            <i class='bx bx-trash' style="font-size: 12px;"></i>
                                                                                        </button>
                                                                                    </div>
                                                                                    <input type="hidden" name="keep_image[]" class="keep-image-input" value="{{ $img->id }}">
                                                                                @endforeach
                                                                            </div>
                                                                        </div>
                                                                    @endif
                                                                    <div class="row add-image-multiple-input">
                                                                        <div class="col-sm-6 mb-3 image-input-wrapper">
                                                                            <div class="d-flex align-items-center">
                                                                                <input type="file" name="attr_image[{{ $index }}][]" class="form-control" placeholder="Enter Product Image">
                                                        ...
                                                        <div class="row mb-3">
                                                            ...
                                                            <div class="col-sm-9">
                                                                ...
                                                                <div class="row add-image-multiple-input">
                                                                    <div class="col-sm-6 mb-3 image-input-wrapper">
                                                                        <div class="d-flex align-items-center">
                                                                            <input type="file" name="attr_image[0][]" class="form-control" placeholder="Enter Product Image">
                                                                            <button type="button" class="btn btn-danger btn-sm ms-2 remove-image-input"><i class='bx bx-trash'></i></button>
                                                                        </div>
                                                                    </div>
                                                                </div>
                            ...
                            <div class="row mb-3">
                                ...
                                <div class="col-sm-9">
                                    <textarea id="desc" name="description" class="form-control" rows="5" placeholder="Enter product description">{{ $data->description }}</textarea>
                                </div>
                            </div>
                            <div class="row">
                                <label class="col-sm-3 col-form-label"></label>
                                <div class="col-sm-9">
                                    <button type="submit" class="btn btn-info px-5">{{ $data->id ? 'Update Product' : 'Add Product' }}</button>
                                </div>
                            </div>
                        </div>
                    </div>
                  ...
                  jQuery.each(result.data, function(key, val){
                     if(val.values && val.values.length > 0) {
                        jQuery.each(val.values, function(attrKey, attrVal){
                           const isSelected = preSelected.includes(attrVal.id) ? 'selected' : '';
                           html += `<option value="${attrVal.id}" ${isSelected}>${val.attribute ? val.attribute.name : val.name} - ${attrVal.value}</option>`;
                        });
                     }
                  });
